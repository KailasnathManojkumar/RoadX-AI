...
# Page Configuration
st.set_page_config(
    page_title="ROADGUARD AI 2.0 | Royal Elite",
    page_icon="⚜️",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Luxurious Royal Black & Gold Theme + Smooth Animations CSS
st.markdown("""
    <style>
        ...
        .main { background-color: #050505; color: #E5E5E5; font-family: 'Inter', sans-serif; }
        .block-container { padding-top: 2rem; padding-bottom: 3rem; max-width: 95%; }
        
        /* Typography */
        h1, h2, h3, h4 { color: #F3E5AB !important; font-family: 'Playfair Display', serif; font-weight: 700; }
        p, span, label, div { color: #D1D5DB; font-family: 'Inter', sans-serif; }
...
with st.sidebar:
    st.markdown("""
        <div style="text-align: center; padding: 15px 0;">
            <h1 style="font-size: 2.5rem; margin-bottom: 0; color: #D4AF37;">⚜️</h1>
            <h2 style="font-size: 1.3rem; letter-spacing: 3px; margin-top: 5px; color: #F3E5AB;">ROADGUARD</h2>
            <p style="font-size: 0.7rem; color: #888888; letter-spacing: 5px; text-transform: uppercase; margin-top: -12px;">AI 2.0 ELITE</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.success("Encrypted Grid Online")
...
if page == "🏛️ Command Center":
    st.markdown("""
        <div class="animated-container">
            <h1 style='text-align: center; margin-bottom: 5px;'>MUNICIPAL SPATIAL COMMAND</h1>
            <p style='text-align: center; color: #9CA3AF; margin-bottom: 30px;'>Autonomous Infrastructure Health & Two-Wheeler Priority Grid</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    m4.metric("AI Confidence", "99.4%", "Optimized")
    ...
    with col_details:
        ...
        <div class="royal-card">
            <b>Location:</b> {r_info['Location']}<br><br>
            <b>Health Index:</b> <span style="color: #D4AF37; font-weight: bold;">{r_info['Health_Score']}/100</span><br><br>
            <b>Structural Status:</b> {r_info['Status']}<br><br>
            <b>Failure Window:</b> {r_info['Predicted_Failure_Days']}<br><br>
            <b>Two-Wheeler Risk:</b> {r_info['Bike_Risk']}
        </div>
        """, unsafe_allow_html=True)
        
        if r_info['Health_Score'] < 30:
            st.error("🚨 **Autonomous Alert:** Priority repair ticket routed.")
        else:
            st.success("✅ **Asset Secure:** Parameters optimal.")

# ================= PAGE 2: NEURAL VISION LAB =================
elif page == "👁️ Neural Vision Lab":
    st.markdown("""
        <div class="animated-container">
            <h1>👁️ Neural Vision Lab</h1>
            <p style='color: #9CA3AF; margin-bottom: 25px;'>Upload surface imagery to execute deep-tensor YOLOv8 structural degradation analysis.</p>
        </div>
    """, unsafe_allow_html=True)
    
    col_up, col_res = st.columns(2)
    with col_up:
        uploaded_img = st.file_uploader("Upload Pavement Target Image", type=["jpg", "jpeg", "png"])
        if uploaded_img:
            st.image(uploaded_img, caption="Target Loaded for Optical Scan", use_container_width=True)
            
    with col_res:
        st.markdown("### ⚜️ Optical Diagnostic Engine")
        if uploaded_img:
            if st.button("Execute High-Precision Scan", type="primary"):
                ...
                <div class="royal-card">
                    ...
                    <b>Sub-base Fatigue:</b> Critical Micro-Fissuring<br>
            ...
            st.info("👈 Please provide an inspection target image on the left panel to run diagnostics.")

# ================= PAGE 3: CITIZEN VIGIL HUB =================
elif page == "📢 Citizen Vigil Hub":
    st.markdown("""
        <div class="animated-container">
            <h1>📢 Citizen Vigil Hub</h1>
            <p style='color: #9CA3AF; margin-bottom: 25px;'>Empowering commuters to crowdsource real-time road degradation data instantly.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with col_feed:
        ...
        for _, row in df.tail(3).iterrows():
            ...
            <div class="royal-card">
                📍 <b>{row['Location']}</b><br>
                Status: {row['Status']} | Rider Risk: {row['Bike_Risk']}<br>
                <span style="font-size: 0.75rem; color: #888888;">Verified via Municipal Grid</span>
            </div>
            """, unsafe_allow_html=True)
...
# ================= PAGE 4: FINANCIAL ANALYTICS =================
    ...
    st.markdown("""
        <div class="animated-container">
            <h1>📊 Financial & Economic Impact</h1>
            <p style='color: #9CA3AF; margin-bottom: 25px;'>Demonstrating the systemic financial transition from reactive patching to preventative AI.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with p1:
        st.markdown("""
        <div class="royal-card">
            <h3 style="color: #ff6b6b !important;">Traditional Reactive Model</h3>
            <h2 style="color: #ff6b6b !important;">₹4.5 Crores / yr</h2>
            <p>High public expenditure caused by waiting for full structural collapse, emergency repaving, and accident damage claims.</p>
        </div>
        """, unsafe_allow_html=True)
    with p2:
        st.markdown("""
        <div class="royal-card">
            ...
            <h2 style="color: #D4AF37 !important;">₹1.8 Crores / yr <span style="font-size: 0.9rem; color: #4CAF50;">(-60% Savings)</span></h2>
            <p>Targeted preventative micro-treatments that triple road asset lifespans at a fraction of standard construction costs.</p>
        </div>
        """, unsafe_allow_html=True)
        
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("### 📥 Official Tender Schedule Export")
...
# ================= PAGE 5: SDG IMPACT & RANKING =================
    ...
    st.markdown("""
        <div class="animated-container">
            <h1>🏆 UN SDG Alignment & Guardian Gamification</h1>
            <p style='color: #9CA3AF; margin-bottom: 25px;'>Aligning municipal innovation with global sustainability targets and civic reward structures.</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with col_sdg1:
        ...
        <div class="royal-card">
            ...
            Maximizes material durability and prevents catastrophic failures through automated early alerts.
        </div>
        """, unsafe_allow_html=True)
    with col_sdg2:
        st.markdown("""
